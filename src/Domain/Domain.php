...
private static $tldTree = array('ac' => array('com' => array(),
'edu' => array(),
'gov' => array(),
'net' => array(),
'mil' => array(),
...
public static function getRegisteredDomain($signingDomain, $fallback = TRUE) {
	...
	if (!strpos($result, '.')) {
		...
		if (!self::validDomainPart($signingDomainParts[$cnt-2]) || !self::validDomainPart($signingDomainParts[$cnt-1])) return NULL;
		return $signingDomainParts[$cnt-2].'.'.$signingDomainParts[$cnt-1];
	}
	return $result;
}

public static function validDomainPart($domPart) {
...
public static function findRegisteredDomain($remainingSigningDomainParts, &$treeNode) {
	...
	if (!self::validDomainPart($sub)) {
		return NULL;
	}

	if (is_array($treeNode) && array_key_exists($sub, $treeNode)) {
		$result = self::findRegisteredDomain($remainingSigningDomainParts, $treeNode[$sub]);
	} else if (is_array($treeNode) && array_key_exists('*', $treeNode)) {
		$result = self::findRegisteredDomain($remainingSigningDomainParts, $treeNode['*']);
	} else {
		return $sub;
	}
